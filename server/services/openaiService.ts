...
export class OpenAIService {
	...
	/**
	 * Generates a step by step plan to run the aplication localy and deploy it to the cloud
	 * @param conversationHistory
	 * @returns Step by step plan to run the application locally and deploy it to the cloud
	 * 
	 */
	async generateDeploymentPlan(conversationHistory: { role: 'user' | 'assistant', content: string }[]
	): Promise<{ deploymentPlan: string, bashCommands: string }> {

		try {
			if (this.isDevelopmentMode()) {
				console.log('Using DEVELOPMENT mode with mock data for OpenAI deployment plan generation');
				return { deploymentPlan: "test", bashCommands: "resr" };

			}
			const prompt = `
        Base on our conversation, generate a comprehensive deployment plan for running the application locally and deploying it to the cloud.
        ...
        1. "deploymentPlan": A markdown-formatted string
        2. "bashCommands": A list of bash commands to run the application locally and deploy it to the cloud that can run in sequence, separated by newlines.`;

			const messages = [
				{ role: 'system', content: 'You are a software deployment expert who creates detailed deployment plans for running applications locally and deploying them to the cloud.' },
				...conversationHistory,
				{ role: 'user', content: prompt }];
			const openai = this.getClient();
			if (!openai) {
			...
			return { deploymentPlan: "default", bashCommands: "default" }
		}
...
- API-First Design`,
        ...
        1. Overall architecture (monolith, microservices, event driven developtment, test driven development, etc.)
        2. Clean architecture principles
        3. Use of design patterns when applicable
        4. Core components
        5. Technology stack for each component (databases, frameworks, libraries, languages, CMS, CRM, etc.)
        6. Devops plan for CI/CD
      ...
      `;
			...
			const messages = [
				{ role: 'system', content: 'You are a software architecture expert who creates detailed architectural plans and starter kit (including all files and dirrectories) based on client requirements.' },
				...conversationHistory,
				{ role: 'user', content: prompt }
			];
